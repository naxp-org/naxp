...
test('the first line names the headers the fragment needs', () => {
	assert.ok(emit('\\A\\9').startsWith('// Needs <cstdint>, <stdexcept>, <string> and <string_view>.'));
});
...
function buildHarness() {
	...
	data.cases.forEach((item, i) => {
		...
		source += `\t{ ${cString(item.naxp)}, ${item.maxEncodedValue}ULL, ${snake}_max_encoded_value, ${snake}_accepts, ${snake}_encode, ${snake}_decode, ${snake}_try_decode, ${snake}_values },\n`;
	});
