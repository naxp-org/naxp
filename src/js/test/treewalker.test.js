...
import { SingleStringOutcome, generates, tryGetSingleString } from '../lib/treewalker.js';
...
test('a decimal range matches the widths its bounds fix', () => {
	const cases = [
		['#[0-10]', '0', true],
		['#[0-10]', '9', true],
		['#[0-10]', '10', true],
...
test('a unified matches whatever its subject accepts', () => {
...
test('a decimal range gives one string only at one number and one width', () => {
	// #[7-07] would be found invalid by W4, so the widths can differ only by way of the values.
...
test('a string too long to build is reported rather than built', () => {
	...
	// takes. The point of the two digit interval cap is that this is invalid without expanding.
