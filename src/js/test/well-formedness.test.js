...
/**
 * Parses and checks, failing the test if the pattern does not parse.
 *
...
function passes(text) {
	...
	assert.equal(error, null, `${text} was invalid: ${error}`);
}

/**
 * Asserts that a naxp is invalid, and returns the fault.
 *
 * @param {string} text The pattern.
...
test('a rendering generating more than one string is invalid', () => {
	const error = breaks('\\s!(B|C)', 'W1');

	assert.ok(error.text.includes('exactly one string'), error.text);
});

test('a rendering the subject never generates is invalid', () => {
...
test('deleting an element whose subject is not optional is invalid', () => {
	const error = breaks('\\s!()', 'W1');
...
test("a '!!' whose subject is not a single string is invalid", () => {
...
test('the message for a bad !! names !! rather than the form it expands to', () => {
	// The whole reason the tree records how a unified was written.
	assert.ok(breaks('\\A!!', 'W1').text.includes("subject of a '!!'"));
	assert.ok(breaks('\\s!(B|C)', 'W1').text.includes("rendering of a '!'"));
});

test('an explicit rendering with two branches is invalid', () => {
	breaks('(A|B)!(A|B)', 'W1');
});
...
test('two unified elements side by side do not nest', () => {
	passes('\\s!! \\s!!');
	passes('(A|b)!b(C|d)!d');
});
...
test('a rendering too long to build breaks W6', () => {
	// (A{99}){99} is one string of 9 801 characters, longer than any naxp within the state
	// budget can generate, so W6 rules it out.
	const error = breaks('((A{99}){99})!!', 'W6');
