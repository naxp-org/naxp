 ...
 *
 * Not a rule of the language. A naxp generating a longer string than this would be found invalid by the
...
function appendSingleString(node, builder) {
	...
	if (node instanceof AstUnified) {
		// The strings x!y generates are the strings x accepts. W2 has already ruled out any tree
...
export function advance(node, text, starts) {
	...
	if (node instanceof AstDecimalRange) { return advanceDecimalRange(node, text, starts); }
 ...
 * @returns {Set<number>} The positions reached.
 */
function advanceDecimalRange(range, text, starts) {
	const result = new Set();
