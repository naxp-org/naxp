...
import { CFamilyEmitter, HELD_NAME, isIdentifier } from './c-family-emitter.js';
	...
	emitEncode(fragment) {
		...
		if (fragment.canonicalises) {
			...
			if (fragment.needsRegister) {
				...
				writer.line(`char ${HELD_NAME}[${fragment.registerDepth}] = { 0 };`);
			}
			...
			if (fragment.needsRegister) {
				...
				writer.line(`${HELD_NAME}[${top}] = text[i];`);
			}
