...
public class CppEmitterTests
{
	...
	public void Emit_NamesTheHeadersItNeeds()
	{
		...
		Assert.StartsWith("// Needs <cstdint>, <stdexcept>, <string> and <string_view>.", source, StringComparison.Ordinal);
	}
	...
	static string BuildHarness()
	{
		...
		for (int i = 0; i < TestData.Cases.Count; ++i)
		{
			...
			builder.Append($"\t{{ {NativeHarness.CString(item.Naxp)}, {item.MaxEncodedValue.ToString(CultureInfo.InvariantCulture)}ULL, {snake}_max_encoded_value, {snake}_accepts, {snake}_encode, {snake}_decode, {snake}_try_decode, {snake}_values }},\n");
		}
