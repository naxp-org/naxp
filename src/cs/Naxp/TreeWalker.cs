...
enum SingleStringOutcome
{
	...
	Multiple,
	/// <summary>It generates one string, but longer than <see cref="TreeWalker.MaxGeneratedLength"/>.</summary>
	TooLong,
}
...
static class TreeWalker
{
	...
	static SingleStringOutcome AppendSingleString(Ast node, StringBuilder builder)
	{
		switch (node)
		{
			...
			case AstUnified unified:
				// The strings x!y generates are the strings x accepts. W2 has already ruled out any
				// tree that reaches this case from within another '!'.
				return AppendSingleString(unified.Subject, builder);
	...
	internal static HashSet<int> Advance(Ast node, ReadOnlySpan<char> text, HashSet<int> starts)
	{
		...
		switch (node)
		{
			...
			case AstDecimalRange range:
				return AdvanceDecimalRange(range, text, starts);
			...
			case AstUnified unified:
				...
				return Advance(unified.Subject, text, starts);
	...
	/// </remarks>
	static HashSet<int> AdvanceDecimalRange(AstDecimalRange range, ReadOnlySpan<char> text, HashSet<int> starts)
	{
		var result = new HashSet<int>();
