...
public sealed class NaxpGenerator : IIncrementalGenerator
{
	...
	/// <summary>
	/// What ends every line of a generated file.
	/// </summary>
	/// <remarks>
	/// Written out rather than taken from <see cref="System.Environment.NewLine"/>, so that the
	/// same sources give the same generated file on every machine. The emitted fragment inside the
	/// wrapper ends its lines the same way, and mixing the two would be worse than either.
	/// </remarks>
		...
		CancellationToken cancellationToken)
	{
		...
		foreach (AttributeData attribute in context.Attributes)
		{
			...
			// The second constructor argument is required, so the compiler has already rejected an
	...
	static void Emit(SourceProductionContext context, TypeModel model)
	{
		...
		builder
			.Append($"// Written by the naxp source generator, version {Emitter.PackageVersion()}. Do not edit.")
			.Append(NewLine);
		...
		{
			builder.Append("namespace ").Append(model.Namespace).Append(NewLine);
		...
		{
			builder.Append(Indent(depth)).Append(header).Append(NewLine);
			builder.Append(Indent(depth)).Append("{").Append(NewLine);
			depth++;
		}
		...
		foreach (NaxpSpec spec in model.Specs)
		{
			...
			if (wroteAny) { builder.Append(NewLine); }
	...
	static bool TryEmit(SourceProductionContext context, NaxpSpec spec, StringBuilder builder, int depth)
	{
		try
		{
			...
			if (compilation!.MaxEncodedValue > Emitter.Capacity(spec.ValueType))
			{
				context.ReportDiagnostic(Rules.Create(
					...
					compilation.MaxEncodedValue.ToString(CultureInfo.InvariantCulture),
					ValueTypes.Keyword(spec.ValueType),
					ValueTypes.Keyword(ValueTypes.Narrowest(compilation.MaxEncodedValue))));
			...
			builder.Append(spec.Prefix.Length == 0 ? "." : $", as {spec.Prefix}.").Append(NewLine);
